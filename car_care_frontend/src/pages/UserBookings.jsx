    ...
    <div>
      ...
      <div className="">
        {bookings.map((item, index) => (
          ...
          >
            <div>
              <img className="w-32 bg-indigo-50" src={`http://localhost:3000${item.serviceCenterData
.imageUrl}`} alt="" />
            </div>
            <div className="flex-1 text-sm text-zinc-600">
              <p className="text-neutral-800 font-semibold">{item.serviceCenterData
.service_center_name}</p>
              <p>{item.serviceType}</p>
              <p className="text-zinc-700 font-medium mt-1">Address:</p>
              <p className="text-xs">{item.serviceCenterData
.service_center_city}</p>
              <p className="text-xs">{item.serviceCenterData
.service_center_state}</p>
            ...
            <div className="flex flex-col gap-2 justify-end">
              ...
              {item.cancelled === 0 && <button 
              onClick={() => handleReSchedule(item.serviceCenterData.serviceCenterId)}
              ...
              {/*if cancelled, then this button*/}
              {
                item.cancelled === 1 && <button className='sm:min-w-48 py-2 border border-red-500 rounded text-red-500'>Booking cancelled</button>
              }
